...
export class CartManagement {
    private allProductCart : ProductCart[] = [];

    addProductCart(pr:ProductCart) : void {
        this.allProductCart.push(pr);     
    };
    checkProductCart(pr: Product) : number {
        for(let i : number = 0; i<this.allProductCart.length; i++){
    ...
    increasingNumberProduct(pr:Product) : void{
        //biết được vị trí của sản phẩm nằm ở đâu trong mảng.
        var locationProduct : number = this.checkProductCart(pr);
        var nowQuantity: number = this.allProductCart[locationProduct].getQuantity();
        this.allProductCart[locationProduct].changeQuantity(nowQuantity + 1);
    }
    viewCart() : ProductCart[] {
        return this.allProductCart;
    }
    updateProductCart(pr:Product, quantity:number) : void {
        for ( let i : number = 0; i < this.allProductCart.length; i++){
    ...
    removeProductCart(pr:Product) : void {
        for ( let i : number = 0; i < this.allProductCart.length; i++){
            var idProduct = this.allProductCart[i].getProduct().id;
            if(idProduct == pr.id){
    ...
    calcPrice() : number {
        var total : number = 0;
        for(let i : number = 0; i<this.allProductCart.length; i++){
            var price : number = this.allProductCart[i].getProduct().price;
            var quantity : number = this.allProductCart[i].getQuantity();
